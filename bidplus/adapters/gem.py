"""GeM portal adapter (S2).

Wraps the existing, tested GeM tool by shelling out to its CLI — never importing
the GeM ``config`` / ``modules`` packages (they use absolute imports that collide
in ``sys.modules`` with the other tools). The subprocess runs with cwd = the GeM
tool dir and an env carrying ``BIDPLUS_RUNTIME_DIR`` + ``ANTHROPIC_API_KEY`` so
GeM's own ``config.py`` resolves its writable state under
``$BIDPLUS_RUNTIME_DIR/gem/``.

Run counts are derived by diffing the GeM ``bids.db`` (opened read-only) before
...
# Repo root: bidplus/adapters/gem.py -> parents[2] == BidAnalysisPortal/
_REPO_ROOT = Path(__file__).resolve().parents[2]
_GEM_DIR = _REPO_ROOT / "gem_portal"


class GeMAdapter:
    """``PortalAdapter`` implementation for the GeM portal."""
    ...
    def _snapshot(self) -> tuple[set[str], int, int]:
        """Return (key set, scored count, CLOSED count) from the GeM DB.
        ...
        try:
            rows = conn.execute(
                "SELECT bid_number, pass1_score, bid_status FROM bids"
            ).fetchall()
        except sqlite3.OperationalError:
            return set(), 0, 0
        finally:
    ...
    def run_pipeline(self) -> RunResult:
        ...
        proc = subprocess.run(
            [sys.executable, "gem_tool.py", "scrape-score"],
            cwd=str(_GEM_DIR),
        ...
        self.last_output = (
            f"$ {sys.executable} gem_tool.py scrape-score\n"
            f"[cwd] {_GEM_DIR}\n"
    ...
    def explain(self, source_pk: str) -> dict:
        proc = subprocess.run(
            [sys.executable, "gem_tool.py", "explain", source_pk],
            cwd=str(_GEM_DIR),
            env=self._subprocess_env(),
            capture_output=True,
            text=True,
        ...
        if proc.returncode != 0:
            raise RuntimeError(
                f"gem_tool.py explain failed (exit {proc.returncode}): "
