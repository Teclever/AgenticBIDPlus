...
class ISROAdapter:
    ...
    def fetch_documents(self, source_pk: str) -> list[FetchedDoc]:
        """Download the tender's single document (+ optional corrigendum) into
        $BIDPLUS_RUNTIME_DIR/isro/bids/<tender_id>/ and return a FetchedDoc per file."""
        from bidplus.adapters.base import fetched_docs_in

        out_dir = str(config.bid_staging_dir(self.portal, source_pk))
        proc = subprocess.run(
            [sys.executable, "isro_tool.py", "fetch-docs", source_pk, "--out", out_dir],
            cwd=str(_ISRO_DIR), env=self._subprocess_env(), capture_output=True, text=True,
        )
        self.last_output = (
            f"$ isro_tool.py fetch-docs {source_pk} --out {out_dir}\n"
            f"[returncode] {proc.returncode}\n--- stdout ---\n{proc.stdout}\n"
            f"--- stderr ---\n{proc.stderr}\n"
        )
        if proc.returncode != 0:
            raise RuntimeError(
                f"isro_tool.py fetch-docs failed (exit {proc.returncode}): "
                f"{proc.stderr.strip() or proc.stdout.strip()}"
            )
