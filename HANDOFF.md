...
| **S5** Shared scoring module + eliminator | centralize Pass-1 out of the tools; normalized-record seam; **two-pass** eliminator gate (`neg & !pos`, read from `eliminator_terms` DB) shadow→hard; ledger + AI-delta + Excel governance; output-budget batching; 1:1 mapping; transient-error retry | **In progress — Chunks 1+2 done + validated (2026-06-05).** ✅ C1 eliminator core (`bidplus/eliminator.py`: schema + idempotent seed (949 terms, guarded=9) + two-pass gate byte-faithful to the miner + shadow analysis; `scoring_records()` seam on all 3 adapters). ✅ C2 Haiku engine (`bidplus/scoring.py`: 1:1 mapping, per-item retry, transient retry/no-skip-to-NULL, no dup-gate; `score` cmd; live smoke 8 GeM bids OK; hard-mode keyword writes OK). ⏳ C3 governance (ledger/AI-delta/Excel), in-tool Pass-1 removal, hard cutover (held — user reviewing the 109 drift collisions in `exports/eliminator_collisions_review.xlsx`). | — |
...
- **Commits on `main`:** `8bd76a6` *docs: eliminator V2 propagation + S0–S4 status* → `f9559cd` *feat(bidplus): orchestrator slices S0–S4* → `a228441` *initial*. (S0–S4 code + all doc updates + eliminator seeds + systemd units are **committed**.)
- **Uncommitted (working tree) — S5 Chunks 1+2, validated:** `bidplus/eliminator.py` (two-pass gate + schema + seed), `bidplus/scoring.py` (normalized record + Haiku Pass-1 engine), `scoring_records()` added to all 3 adapters, launcher `eliminate`/`score` commands. *(Runtime-only, never committed: `parent.db` now also holds the seeded `eliminator_terms`/`eliminator_keyword_stats`/`list_change_proposals`; GeM tool DB has 8 live model-scores from the smoke.)*
