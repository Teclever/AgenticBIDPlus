...
export function BidDetail() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-3 flex-wrap">
            ...
            <ScoreCard rating={bid.rating} method={bid.method} />
            {boostTerm && (
              <span
                title={`Auto-promoted to 5/5 — matched boost keyword “${boostTerm}”`}
                className="shrink-0 self-center inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-purple-50 border border-purple-300 text-purple-800 rounded-full"
              >
      ...
      <section className="bg-white rounded-xl border border-gray-200 p-6">
        <h2 className="text-base font-semibold text-gray-900 mb-3">AI Rating</h2>
        <RatingDisplay
          rating={bid.rating}
          method={bid.method}
        ...
        {bid.method === "model" && rationaleText && (
          <details className="mt-3">
            <summary className="text-sm text-gray-500 cursor-pointer hover:text-gray-700 select-none">
              Why this rating?{" "}
              <span className="text-gray-400">(explains the score, not the tender contents)</span>
            </summary>
            <p className="text-sm text-gray-600 mt-2">{rationaleText}</p>
          </details>
        )}
      </section>
...
function ScoreCard({ rating, method }: { rating: number | null; method: "model" | "keyword" }) {
  const filtered = method === "keyword";
  const score = filtered ? 0 : rating;
  const palette = filtered
  ...
  return (
    <div className={`shrink-0 flex flex-col items-center px-3 py-1.5 rounded-lg border ${palette}`}>
      <span className="text-[10px] font-medium uppercase tracking-wide opacity-70">AI Score</span>
      {filtered ? (
        <span className="text-sm font-bold leading-tight">Filtered</span>
      ) : (
        <span className="text-lg font-bold leading-tight">{score ?? "—"}<span className="text-xs font-medium opacity-60">/5</span></span>
      )}
    </div>
  );
}
