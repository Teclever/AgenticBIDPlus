...
| **Dashboard** | `/` | **Working (updated 2026-06-19)** | **Four** portal cards (`md:grid-cols-2 xl:grid-cols-4`): GEM, HAL e-Procurement, **HAL Corporate Tenders (`halc`)**, ISRO. Opportunity Distribution chips show **new / accepted** separately per score band (rejected excluded). Top stat tiles = New / Accepted. **"Single / Limited Tender"** amber chip (when > 0). Closing Soon note. |
| **Bid list** | `/portal/:portalId` | **Working (updated 2026-06-19)** | Sorted newest-first. Inline Accept/Reject on `new` rows; **inline "Reset to New" on accepted/rejected rows** (table + card). "Single / Limited Tender" badge (green ★ Teclever, amber others). Filter `singletender`. `rfilter/rpage/ridx` for next-bid nav. |
| **Bid detail** | `/portal/:portalId/bid/:bidKey` | **Working (updated 2026-06-19)** | Accept/Reject navigates to next bid; generation-counter prevents stale state. **"Reset to New" now shown for ANY disposed bid (accepted or rejected)**. Documents on-demand download (HALC follows e-proc `T_RETURN_URL`). "Single / Limited Tender" banner (green/amber + seller list). |
