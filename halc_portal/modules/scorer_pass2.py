...
For a single shortlisted HAL tender: downloads all document links (and best-effort
...
from modules.fetcher import collect_doc_links
...
def score_bid_pass2(bid: dict, capability_reference: str) -> dict | None:
    ...
    session.headers.update(
        {"User-Agent": USER_AGENT, "Referer": "https://hal-india.co.in/tender"}
    )

    links = _collect_links(session, bid)
    ...
    prompt = (
        "Score this HAL tender after reviewing all provided documents.\n"
...
def _collect_links(session: requests.Session, bid: dict) -> list[str]:
    """Resolve document links live at Pass 2 time.

    Delegates to the portal adapter (`fetcher.collect_doc_links`), which fetches
    the tender detail page fresh and returns `document_url` + `corrigendum_url` +
    the detail-page document anchors. This replaces the previously pre-populated
    `doc_links_json` and yields the most current document set. Falls back to the
    listing-derived URLs if the detail fetch/parse fails.
    """
    try:
        return collect_doc_links(session, bid)
    except Exception:
        out: list[str] = []
        seen: set[str] = set()
