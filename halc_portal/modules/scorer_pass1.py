...
from config import PASS1_BATCH_SIZE, PASS1_DETAIL_CHARS, PASS1_MODEL

MAX_BATCH_RETRIES = 2


def build_pass1_prompt(bids: list[dict], cap_ref: str, detail_chars: int = PASS1_DETAIL_CHARS) -> str:
    """Deterministically assemble the Pass-1 prompt for a set of tenders (no API call).

    Mirrors the prompt built in ``_score_chunk`` so the ``explain`` dry-run shows
    exactly what the scorer would send. Standalone Pass-1 is centralized in
    bidplus.scoring for orchestrated runs; this is for manual validation."""
    payload = [
        {
            "tender_id": b.get("tender_id"),
            "center_name": b.get("center_name", ""),
            "tender_description": b.get("tender_description", ""),
            "detail_text": (b.get("detail_text") or "")[:detail_chars],
        }
        for b in bids
    ]
    return (
        "Score each HAL tender from 0 to 5 for Teclever fit.\n"
    ...
    payload = [
        {
            ...
            "center_name": b.get("center_name", ""),
            "tender_description": b.get("tender_description", ""),
            "detail_text": (b.get("detail_text") or "")[:PASS1_DETAIL_CHARS],
        }
        for b in bids
    ]
    prompt = (
